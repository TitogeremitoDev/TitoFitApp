...
const ExerciseItem = memo(
  ({
    ...
    openId,
    theme,
    exerciseDetails, // passed directly
    setOpenId,
    setEstadoEjLocal,
    onOpenTC,
    onOpenImage,
    ...
    onOpenNotes,
    onScrollTo,
    notes, // We need notes prop too!
    nextItem, // We need nextItem to check for biserie
  }) => {
    if (!item) return null;
    ...
    const isBiserie = (ej) => {
      ...
      return ej.series.some((serie) => {
        const extra = String(serie?.extra || '')
          .toLowerCase()
          .trim();
        ...
        >
          ...
          <TouchableOpacity
            style={[
              styles.cardHeader,
              {
                borderColor: theme.cardHeaderBorder,
          ...
          <View style={styles.stateToolsRow}>
            ...
            <View style={styles.toolsRow}>
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <Text style={[styles.toolBtnTxt, { color: theme.text }]}>
                  TC
                </Text>
              </TouchableOpacity>
            ...
            <View style={styles.seriesBox}>
              <View style={styles.serieRowHeader}>
                ...
                <View style={styles.inputCol}>
                  <Text
                    style={[styles.colLabel, { color: theme.textSecondary }]}
                  >
                    Reps
                  </Text>
                </View>
              ...
              {(item.series || []).map((serie, idx) => {
                ...
                const prevReps = findPrevData(prog, semana, diaIdx, item.id, idx, 'reps');
                const prevKg = findPrevData(prog, semana, diaIdx, item.id, idx, 'peso');
                ...
                const isFallo =
                  String(repMinRaw).toLowerCase() === 'fallo' ||
                  String(repMaxRaw).toLowerCase() === 'fallo';

                const repMin =
                  !isFallo && repMinRaw != null ? Number(repMinRaw) : null;
                const repMax =
                  !isFallo && repMaxRaw != null ? Number(repMaxRaw) : null;
                  ...
                  >
                    <View style={{ width: 70, justifyContent: 'center' }}>
                      ...
                      {serie?.nota && serie.nota.trim() !== '' && (
                        <TouchableOpacity
                          onPress={() =>
                            Alert.alert('📝 Nota del Coach', serie.nota)
                          }
                          style={{ marginTop: 3 }}
                          activeOpacity={0.7}
                        >
                    ...
                    {/* SP flag */}
                    {prevExceeded && (
                      <Text style={[styles.sp, { color: theme.primary }]}>
                        ¡SP!
                      </Text>
                    )}

                    <Text
                      style={[styles.extraTxt, { color: theme.textSecondary }]}
                    >
  ...
  (prev, next) => {
    // Custom arePropsEqual for performance

    // 1. Identity checks (cheap)
    if (prev.item.id !== next.item.id) return false;
    if (prev.index !== next.index) return false;
    if (prev.semana !== next.semana) return false;
    if (prev.diaIdx !== next.diaIdx) return false;

    // Check if open state changed for THIS item
    const prevOpen = prev.openId === prev.item.id;
    const nextOpen = next.openId === next.item.id;
    if (prevOpen !== nextOpen) return false;

    // Check theme ref (usually stable)
    if (prev.theme !== next.theme) return false;

    // Check if next item changed (for biserie connector)
    if (prev.nextItem?.id !== next.nextItem?.id) return false;

    // Check notes for this exercise's series
    // If notes object ref changed, we need to check if notes for THIS exercise changed
    if (prev.notes !== next.notes) {
       const series = prev.item.series || [];
       for (let i = 0; i < series.length; i++) {
         const key = `${prev.semana}|${prev.diaIdx}|${prev.item.id}|${i}`;
         if (prev.notes[key] !== next.notes[key]) return false;
       }
    }

    // 2. Prog checks (the heavy lifting)
    // Check status
    const statusKey = `${prev.semana}|${prev.diaIdx}|${prev.item.id}`;
    if (prev.prog[statusKey] !== next.prog[statusKey]) return false;

    // Check series inputs
    // If input is open, we need to be responsive.
    // If not open, we might skip? No, we should always be correct.
    if (nextOpen) { // Only if open
      const series = prev.item.series || [];
      for (let i = 0; i < series.length; i++) {
        const key = `${statusKey}|${i}`;
        // Series object reference check {reps: '10', peso: '20'}
        if (prev.prog[key] !== next.prog[key]) return false;

        // Also check historical data if findPrevData logic depends on external factors?
        // findPrevData depends on prog.
        // We assume historical data (prev weeks) doesn't change during session.
      }
    }

    return true; // No changes detected, skip render
  }
);
