...
import { EnhancedTextInput as TextInput } from '../ui';
import { EXTRA_ABBR, NOTE_VALUES, getTrendIcon, findPrevHelper } from './ExerciseCardUtils';
...
const ExerciseCard = memo(({
        ...
        <View style={styles.stateToolsRow}>
          <View style={styles.stateRow}>
            ...
            <TouchableOpacity
              ...
              onPress={() => onSetEstado(ejerKey, 'NC')}
            >
              <Ionicons
                name="close"
                size={16}
            ...
            <TouchableOpacity
              ...
              onPress={() => onSetEstado(ejerKey, 'OE')}
            >
              <Ionicons
                ...
                <View key={idx} style={[styles.serieRow, {
                  ...
                  <View style={styles.inputWithTrend}>
                    <TextInput
                      ...
                      onChangeText={(v) => onSetSerieDato(serieKey, 'reps', v, item, (item.series || []).length)}
                  ...
                  <View style={styles.inputWithTrend}>
                    <TextInput
                      ...
                      keyboardType="numeric"
                      value={curr.peso || ''}
                      onChangeText={(v) => onSetSerieDato(serieKey, 'peso', v, item, (item.series || []).length)}
                      onBlur={flushProgress}
                    />
                    {iconKg && (
                      <Ionicons
  ...
  const isSameItem = prev.item.id === next.item.id;
  const isSameExpanded = prev.isExpanded === next.isExpanded;
  const isSameBiserie = prev.isBiserie === next.isBiserie;
  const isSameConnector = prev.showBiserieConnector === next.showBiserieConnector;
  const isSameTheme = prev.theme === next.theme; // Theme might change? Assume stable reference if from context, but checking is safe.
  const isSameSemana = prev.semana === next.semana;
  const isSameDia = prev.diaIdx === next.diaIdx;

  if (!isSameItem || !isSameExpanded || !isSameBiserie || !isSameConnector || !isSameTheme || !isSameSemana || !isSameDia) {
    return false;
  }

  // Check prog for this item
  const ejerKey = `${next.semana}|${next.diaIdx}|${next.item.id}`;
  if (prev.prog[ejerKey] !== next.prog[ejerKey]) return false;

  // Check series
  if (next.item.series) {
    for (let i = 0; i < next.item.series.length; i++) {
      const serieKey = `${ejerKey}|${i}`;
      // Check reps/weight in prog
      if (prev.prog[serieKey] !== next.prog[serieKey]) return false;
      // Check notes
      // notes structure: { [serieKey]: { value, note, ... } }
      const prevNote = prev.notes?.[serieKey];
      const nextNote = next.notes?.[serieKey];
      if (prevNote !== nextNote) return false;
    }
  }

  // Check exerciseDetails
