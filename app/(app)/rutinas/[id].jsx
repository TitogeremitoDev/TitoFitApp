...
const SerieRow = React.memo(({ diaKey, ejercicioId, s, index, updateSerieCampo, toggleSerieExtra, toggleFallo, deleteSerie }) => {
  ...
  return (
    <View style={[styles.serieRow, isBiserie && styles.serieRowBiserie]}>
      ...
      <View style={styles.colFallo}>
        ...
        >
          <Text style={[styles.falloBtnTxt, isFallo && styles.falloBtnTxtActive]} numberOfLines={1}>Fallo</Text>
        </TouchableOpacity>
      </View>
    ...
    try {
      ...
      if (data.success) {
        ...
        if (isLocalId && data.routine?._id && method === 'POST') {
          ...
          // 🐛 FIX: Para actualizaciones (PUT), también cachear los cambios localmente
          await AsyncStorage.setItem(`routine_${id}`, JSON.stringify(rutina));

          // Actualizar también el nombre en la lista de rutinas
          const rutinasStr = await AsyncStorage.getItem('rutinas');
          if (rutinasStr) {
            const rutinas = JSON.parse(rutinasStr);
            const updatedRutinas = rutinas.map(r =>
          ...
          console.log('[Rutinas] ✅ Rutina actualizada y cacheada localmente:', id);
        }
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setTechModal({ visible: false, title: '', tips: [] })}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.techModalCard}>
            <View style={styles.techModalHeader}>
              <Text style={styles.techModalTitle} numberOfLines={2}>
                📖 {techModal.title}
              </Text>
              <Pressable
                onPress={() => setTechModal({ visible: false, title: '', tips: [] })}
                style={styles.techModalCloseBtn}
              >
                <Ionicons name="close-outline" size={24} color="#1e293b" />
              </Pressable>
            </View>
            <ScrollView style={styles.techModalContent}>
              {techModal.tips.map((tip, idx) => (
                <View key={idx} style={styles.techTipRow}>
                  <Text style={styles.techTipBullet}>•</Text>
                  <Text style={styles.techTipText}>{tip}</Text>
                </View>
              ))}
            </ScrollView>
          </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.imageModalCard}>
            <View style={styles.imageModalHeader}>
              <Text style={styles.imageModalTitle} numberOfLines={2}>
                📸 {imageModal.title}
              </Text>
              <Pressable
...
const styles = StyleSheet.create({
  ...
  extraPillTxtCompact: { fontSize: 11, fontWeight: '600', color: '#475569' },
